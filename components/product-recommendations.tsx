...
export default function ProductRecommendations({ placement, productIds, titleBg, titleEn }: ProductRecommendationsProps) {
  ...
  const title = locale === "en" ? titleEn ?? "This might come in handy" : titleBg ?? "Може да ти свърши работа";
  const viewAll = locale === "en" ? "View all →" : "Виж всички →";
  const disclaimer =
    locale === "en"
  ...
  return (
    <div className="mt-10 border-t border-white/10 pt-8">
      <div className="flex items-center justify-between">
        <p className="text-xs uppercase tracking-widest text-chalkDim">{title}</p>
      ...
      <p className="mt-3 text-[11px] text-chalkDim">{disclaimer}</p>
    </div>
  );
}
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {product.highlight && (
          <span className="shrink-0 border border-amber/40 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber">
            {HIGHLIGHT_LABEL[product.highlight][locale]}
          </span>
        )}
      </div>
      <p className="mt-1 text-xs leading-relaxed text-chalkDim">{product.note[locale]}</p>
    </a>
  );
}

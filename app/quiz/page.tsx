...
export default function QuizPage() {
  ...
  async function handleSelect(value: unknown) {
    ...
    if (error || !data) {
      setErrorMessage("Не успяхме да заредим каталога с програми. Провери връзката и опитай пак.");
      setPhase("error");
      return;
    }
      ...
      <div className="mx-auto max-w-2xl">
          ...
          <>
            ...
            <div className="mb-10">
              <div className="flex items-center justify-between text-xs uppercase tracking-widest text-chalkDim">
                <span>
                  Въпрос {stepIndex + 1} от {QUIZ_QUESTIONS.length}
                </span>
        ...
        {phase === "loading" && (
          <div className="flex flex-col items-center gap-4 py-24 text-center">
            ...
            <p className="text-chalkDim">Изчисляваме най-подходящите програми за теб…</p>
          </div>
        )}
        ...
        {phase === "results" && (
          <div>
            <h1 className="font-display text-2xl font-semibold md:text-3xl">
              Ето какво ти препоръчваме
            </h1>
            <p className="mt-2 text-chalkDim">
              Подредени по това колко добре пасват на отговорите ти.
            </p>

            <div className="mt-10 grid gap-6">
              {results.length === 0 && (
                <p className="text-chalkDim">
                  Нито една програма не пасва достатъчно добре на тази комбинация — пробвай да
                  промениш някой отговор.
                </p>
              )}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <span className="font-display text-[11px] uppercase tracking-widest text-amber">
                        {i === 0 ? "Най-добро попадение" : `#${i + 1} по подходящост`} ·{" "}
                        {r.matchLevel} мач
                      </span>
                      <h2 className="mt-1 font-display text-xl font-semibold">{r.program.name}</h2>
                    </div>
                    <span className="font-display text-3xl font-bold text-steelLight">
                      {r.score}
                    </span>
                  </div>

                  <p className="mt-3 text-sm text-chalkDim">
                    {r.program.recommendationProfile.pitch}
                  </p>
